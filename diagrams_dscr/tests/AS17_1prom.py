...
with Diagram(
    ...
    with Cluster("SBERTECH_AD", direction="LR"):
        with Cluster(f"VDC18 \n{network}", direction="LR"):
            ...
            sowa = SOWA(
                "SOWA: \n vsp-ac17.1-sowa  \n  10.44.18.4\n sowa.ac17-1.common.sbt",
                fill="lightblue",
                stroke="red",
                labeltooltip="Для безопасности",
            )
        ...
        with Cluster(
            ...
            with Cluster("TEST AC17.1", graph_attr={"bgcolor": "#ECE8F6"}):
                ...
                (
                    apidev
                    >> Edge(
                        label="Вызовы API для функций",
                        headlabel="https:8443 ",
                        decorate="true",
                        style="diagonals",
                        color="black",
                    )
            ...
            (
                apidev
                >> Edge(
                    headlabel="https://cmdb.infra.sbt/ ",
                    decorate="true",
                    style="dashed",
                    color="yellow",
                )
    ...
    with Cluster("In", direction="LR", graph_attr={"bgcolor": "#ECE8F6"}):
        ...
        with Cluster("Other AS", direction="LR", graph_attr={"bgcolor": "#EAFF59"}):
            ...
            avpo = EC2Instance(
                "AC 34/ AVPO"
            )  # https://itsm.infra.sbt/sd/operator/#uuid:serviceCall$31150971:serviceCall$request
            pam = EC2Instance(" AC 32 / PAM \n 10.50.13.35\n 10.50.13.36")
            SOCSIEM = EC2Instance("AC 38/ SOC/SIEM\n TBD ")
            ConfluenceDZO = Confluence(
    ...
    (
        vdb
        >> Edge(
            label="CC-14: Индексация",
            style="bold",
            decorate="true",
            color="red",
            headlabel="https:cmdb.infra.sbt:443",
        )
        >> as17
    )
    ...
    (
        vdb
        >> Edge(
            label="Индексация документов Confluence",
            style="normal",
            decorate="true",
            color="red",
            headlabel="httops://sberworks.ru/wiki:443",
        )
        >> ConfluenceDZO
    )
    ...
    (
        sowa
        >> Edge(
            label="Доступ до Gigachat: AUTH + API",
    ...
    (
        ...
        >> Edge(
            ...
            style="dashed",
            decorate="true",
            description="Доступ пользователей в отладочных целях",
        )
        >> api
    )
    ...
    (
        ...
        >> dlp
    )  # https://10.50.13.11:1345/icap/full
